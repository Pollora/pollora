...
do_action( 'woocommerce_before_account_payment_methods', $has_methods ); @endphp
...
@foreach ( wc_get_account_payment_methods_columns() as $column_id => $column_name )
					...
					<th class="woocommerce-PaymentMethod woocommerce-PaymentMethod--{!! esc_attr( $column_id ) !!} payment-method-{!! esc_attr( $column_id ) !!}"><span class="nobr">{!! esc_html( $column_name ) !!}</span></th>
...
@foreach ( wc_get_account_payment_methods_columns() as $column_id => $column_name )
						...
						<td class="woocommerce-PaymentMethod woocommerce-PaymentMethod--{!! esc_attr( $column_id ) !!} payment-method-{!! esc_attr( $column_id ) !!}" data-title="{!! esc_attr( $column_name ) !!}">
...
@php
							if ( has_action( 'woocommerce_account_payment_methods_column_' . $column_id ) ) {
								...
								foreach ( $method['actions'] as $key => $action ) { 									echo '<a href="' . esc_url( $action['url'] ) . '" class="button ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a>&nbsp;';
								}
							}
							@endphp
						</td>
...
@if ( WC()->payment_gateways->get_available_payment_gateways() )
	...
	<a class="button" href="{!! esc_url( wc_get_endpoint_url( 'add-payment-method' ) ) !!}">@php esc_html_e( 'Add payment method', 'woocommerce' ); @endphp</a>
