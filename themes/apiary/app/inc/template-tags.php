    ...
    function entry_footer()
    {
        ...
        if ('post' === get_post_type()) {
            ...
            if ($categories_list) {
                ...
                printf(
                    '<span class="cat-links">'.esc_html__('Posted in %1$s', 'apiary').'</span>',
                    $categories_list
                );
            }
            ...
            if ($tags_list) {
                ...
                printf(
                    '<span class="tags-links">'.esc_html__('Tagged %1$s', 'apiary').'</span>',
                    $tags_list
                );
            }
        }
            ...
            comments_popup_link(
                sprintf(
                    wp_kses(
                        ...
                        __('Leave a Comment<span class="screen-reader-text"> on %s</span>', 'apiary'),
                        [
                            'span' => [
                                'class' => [],
                            ],
...
if (! function_exists('comments_title')) {
    ...
    function comments_title($count)
    {
        if (1 === $count) {
            return sprintf(
                esc_html__('One thought on &ldquo;%1$s&rdquo;', 'apiary'),
                '<span>'.get_the_title().'</span>'
            );
        }

        return sprintf(
            esc_html(_nx('%1$s thought on &ldquo;%2$s&rdquo;', '%1$s thoughts on &ldquo;%2$s&rdquo;', $count, 'comments title', 'apiary')),
