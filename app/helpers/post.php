...
if (! function_exists('posted_on')) {
    ...
    function posted_on()
    {
        ...
        $posted_on = sprintf(
            esc_html_x('Posted on %s', 'post date', APP_TD),
...
if (! function_exists('posted_by')) {
    ...
    function posted_by()
    {
        ...
        $byline = sprintf(
            esc_html_x('by %s', 'post author', APP_TD),
    ...
    function entry_footer()
    {
        ...
        if ('post' === get_post_type()) {
            ...
            if ($categories_list) {
                ...
                printf(
                    '<span class="cat-links">'.esc_html__('Posted in %1$s', APP_TD).'</span>',
                    $categories_list
                );
            }

            /* translators: used between list items, there is a space after the comma */
            $tags_list = get_the_tag_list('', esc_html_x(', ', 'list item separator', APP_TD));

            if ($tags_list) {
                /* translators: 1: list of tags. */
                printf(
                    '<span class="tags-links">'.esc_html__('Tagged %1$s', APP_TD).'</span>',
                    $tags_list
                );
            }
        }
            ...
            comments_popup_link(
                sprintf(
                    wp_kses(
                        ...
                        __('Leave a Comment<span class="screen-reader-text"> on %s</span>', APP_TD),
                        [
                            'span' => [
                                'class' => [],
                            ],
        ...
        edit_post_link(
            sprintf(
                wp_kses(
                    ...
                    __('Edit <span class="screen-reader-text">%s</span>', APP_TD),
                    [
                        'span' => [
                            'class' => [],
                        ],
...
if (! function_exists('comments_title')) {
    ...
    function comments_title($count)
    {
        ...
        return sprintf(
            esc_html(_nx('%1$s thought on &ldquo;%2$s&rdquo;', '%1$s thoughts on &ldquo;%2$s&rdquo;', $count, 'comments title', APP_TD)),
...
if (! function_exists('archive_content_message')) {
    ...
    {
        return sprintf(
            '<p>'.esc_html__('Try looking in the monthly archives. %1$s', APP_TD).'</p>',
            convert_smilies(':)')
        );
    }
}
